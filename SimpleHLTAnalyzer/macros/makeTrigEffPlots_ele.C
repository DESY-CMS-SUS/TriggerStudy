...
void runTrigEffPlots_ele(std::vector<TString> infileNames)
{
  ...
  unsigned int passtrig1 = 0;  chain->SetBranchAddress("HLT_Ele15_IsoVVVL_PFHT600_v1", &passtrig1);
  unsigned int passtrig2 = 0;  chain->SetBranchAddress("HLT_Ele27_eta2p1_WP75_Gsf_TriCentralPFJet30_v1", &passtrig2);
  unsigned int passtrig3 = 0;  chain->SetBranchAddress("HLT_Ele15_IsoVVVL_PFHT400_PFMET70_v1", &passtrig3);
  unsigned int passtrig4 = 0;  chain->SetBranchAddress("HLT_Ele27_eta2p1_WP85_Gsf_HT200_v1", &passtrig4);
  unsigned int passtrig5 = 0;  chain->SetBranchAddress("HLT_Ele27_eta2p1_WP75_Gsf_TriCentralPFJet50_40_30_v1", &passtrig5);

  // auxiliary trigger decision
  unsigned int passaux = 0;   chain->SetBranchAddress("HLT_Ele40_eta2p1_WP85_Gsf_v1", &passaux);
  ...
  // Trigger objects (electrons)
  int nhltelectrons = 0;          chain->SetBranchAddress("nhltelectrons", &nhltelectrons);
  float hltelectronpt[maxsize];   chain->SetBranchAddress("hltelectronpt", &hltelectronpt);
  float hltelectroneta[maxsize];  chain->SetBranchAddress("hltelectroneta", &hltelectroneta);
  float hltelectronphi[maxsize];  chain->SetBranchAddress("hltelectronphi", &hltelectronphi);

  int nelectrons = 0;          chain->SetBranchAddress("nelectrons", &nelectrons);
  float electronpt[maxsize];   chain->SetBranchAddress("electronpt", &electronpt);
  float electroneta[maxsize];  chain->SetBranchAddress("electroneta", &electroneta);
  float electronphi[maxsize];  chain->SetBranchAddress("electronphi", &electronphi);
  ...
  for(int ievent = 0; ievent < nevents; ievent++) {
    ...
    for(int ijet = 0; ijet < ngenjets; ijet++) {
      if(fabs(genjeteta[ijet]) < 2.4 && genjetpt[ijet] > 30.0) {
        bool overlap = false;
       HT = HT+genjetpt[ijet];
        // check whether there is overlap with the electron trigger object which fires the auxiliary trigger ... if so, don't count the jet
        for(int imu = 0; imu < nhltelectrons; imu++) {
          if(deltaR(hltelectroneta[imu], hltelectronphi[imu], genjeteta[ijet], genjetphi[ijet]) < 0.4) {
            overlap = true;
          }
        }
       // if(!overlap) {
      ...
      for(int imu = 0; imu < nelectrons; imu++) {
       if(fabs(electroneta[imu])< 2.1){
       if(electronpt[imu] > Mu1pt) {
          Mu1pt = electronpt[imu];
          }
       if(electronpt[imu] > 25) 
          Nmu ++;
         else if (electronpt[imu] > 10) Nmu_V ++;
     }
    ...
    int npassmu = 0;
    for(int imu = 0; imu < nhltelectrons; imu++) {
      if(hltelectronpt[imu] > 50.0 && fabs(hltelectroneta[imu]) < 2.1) { npassmu++; }
    }
...
TGraphAsymmErrors *effvsMet_4 =makeEffGraph(hMet_PassTrig_4, hMet);
  ...
  effvsMet->SetMarkerSize(0.9);
  effvsMet->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_MET.pdf");
 ...
 InitGraph(effvsST,"HLT_PFHT350_PFMET120_NoiseCleaned_v1","S_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  ...
  effvsST->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_ST.pdf");
  ...
  effvsHT->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_HT.pdf");
 ...
 InitGraph(effvsMet_0,"HLT_PFHT900_v1","#slash{E}_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  ...
  effvsMet_0->SetMarkerSize(0.9);
  effvsMet_0->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_MET_0.pdf");
 ...
 InitGraph(effvsST_0,"HLT_PFHT900_v1","S_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  ...
  effvsST_0->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_ST_0.pdf");
  ...
  effvsHT_0->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_HT_0.pdf");


InitGraph(effvsMet_1,"HLT_Ele15_IsoVVVL_PFHT600_v1","#slash{E}_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  effvsMet_1->SetMarkerStyle(kFullCircle);
  effvsMet_1->SetMarkerSize(0.9); 
  effvsMet_1->SetMarkerStyle(kFullCircle);
  effvsMet_1->SetMarkerSize(0.9);
  effvsMet_1->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_MET_1.pdf");

 InitGraph(effvsST_1,"HLT_Ele15_IsoVVVL_PFHT600_v1","S_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  effvsST_1->SetMarkerStyle(kFullCircle);
  effvsST_1->SetMarkerSize(0.9);
  effvsST_1->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_ST_1.pdf");

  InitGraph(effvsHT_1,"HLT_Ele15_IsoVVVL_PFHT600_v1","HT [GeV]","Efficiency",600,3000.0,0.0,1.1,kRed);
  effvsHT_1->SetMarkerStyle(kFullCircle);
  effvsHT_1->SetMarkerSize(0.9);
  effvsHT_1->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_HT_1.pdf");
...
InitGraph(effvsMet_2,"HLT_Mu15_IsoVVVL_PFHT600 || HLT_Mu15_IsoVVVL_PFHT400_PFMET70","#slash{E}_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  ...
  effvsMet_2->SetMarkerSize(0.9);
  effvsMet_2->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_MET_2.pdf");
 ...
 InitGraph(effvsST_2,"HLT_Mu15_IsoVVVL_PFHT600 || HLT_Mu15_IsoVVVL_PFHT400_PFMET70","S_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  ...
  effvsST_2->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_ST_2.pdf");
  ...
  effvsHT_2->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_HT_2.pdf");


InitGraph(effvsMet_3,"HLT_Ele15_IsoVVVL_PFHT400_PFMET70_v1","#slash{E}_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  effvsMet_3->SetMarkerStyle(kFullCircle);
  effvsMet_3->SetMarkerSize(0.9); 
  effvsMet_3->SetMarkerStyle(kFullCircle);
  effvsMet_3->SetMarkerSize(0.9);
  effvsMet_3->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_MET_3.pdf");

 InitGraph(effvsST_3,"HLT_Ele15_IsoVVVL_PFHT400_PFMET70_v1","S_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  effvsST_3->SetMarkerStyle(kFullCircle);
  effvsST_3->SetMarkerSize(0.9);
  effvsST_3->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_ST_3.pdf");

  InitGraph(effvsHT_3,"HLT_Ele15_IsoVVVL_PFHT400_PFMET70_v1","HT [GeV]","Efficiency",600,3000.0,0.0,1.1,kRed);
  effvsHT_3->SetMarkerStyle(kFullCircle);
  effvsHT_3->SetMarkerSize(0.9);
  effvsHT_3->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_HT_3.pdf");

InitGraph(effvsMet_4,"HLT_Ele27_eta2p1_WP85_Gsf_HT200_v1","#slash{E}_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  effvsMet_4->SetMarkerStyle(kFullCircle);
  effvsMet_4->SetMarkerSize(0.9); 
  effvsMet_4->SetMarkerStyle(kFullCircle);
  effvsMet_4->SetMarkerSize(0.9);
  effvsMet_4->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_MET_4.pdf");

 InitGraph(effvsST_4,"HLT_Ele27_eta2p1_WP85_Gsf_HT200_v1","S_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  effvsST_4->SetMarkerStyle(kFullCircle);
  effvsST_4->SetMarkerSize(0.9);
  effvsST_4->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_ST_4.pdf");

  InitGraph(effvsHT_4,"HLT_Ele27_eta2p1_WP85_Gsf_HT200_v1","HT [GeV]","Efficiency",600,3000.0,0.0,1.1,kRed);
  effvsHT_4->SetMarkerStyle(kFullCircle);
  effvsHT_4->SetMarkerSize(0.9);
  effvsHT_4->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_HT_4.pdf");
...
InitGraph(effvsMet_5,"HLT_Mu15_IsoVVVL_PFHT600 || HLT_Mu15_IsoVVVL_PFHT400_PFMET70 || HLT_PFMET120_NoiseCleaned_Mu5","#slash{E}_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  ...
  effvsMet_5->SetMarkerSize(0.9);
  effvsMet_5->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_MET_5.pdf");
 ...
 InitGraph(effvsST_5,"HLT_Mu15_IsoVVVL_PFHT600 || HLT_Mu15_IsoVVVL_PFHT400_PFMET70|| HLT_PFMET120_NoiseCleaned_Mu5 ","S_{T} [GeV]","Efficiency",0.0,800.0,0.0,1.1,kRed);
  ...
  effvsST_5->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_ST_5.pdf");
  ...
  effvsHT_5->Draw("AP");
 c->SaveAs("./fig_El/eff_trig_vs_HT_5.pdf");
...
void makeTrigEffPlots_ele()
{

  std::vector<TString> filelist;

//  filelist.push_back("hlt_T1tttt.root");
